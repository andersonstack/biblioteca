...
class EmprestimoComponent extends HTMLElement {
  ...
  private render() {
    this.shadow.innerHTML = `
            ...
            <form>
                <h2>ID do Livro</h2>
                <my-input class="input__preencher" id="id-livro" placeholder="Livro" aria-label="ID do Livro"></my-input>
                <h2>UserName do usuário</h2>
                <my-input class="input__preencher" id="id-usuario" placeholder="Usuário" aria-label="ID do usuário"></my-input>
  ...
  private setup() {
    this.shadow
      ...
      .addEventListener("click", async () => {
        const inputIdLivro = this.shadow.querySelector(
          "my-input#id-livro"
        ) as HTMLInputElement;
